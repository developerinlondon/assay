...
- `c.kv:get(mount, key)` -> `{data}`|nil -- Read KV v2 secret. `mount` = engine mount (e.g.
  `"secrets"`)
...
- `c.sys:health()` -> `{initialized, sealed, version, ...}` -- Get Vault health (works even when
  sealed)
...
- `c.engines:enable(path, type, opts?)` -> nil -- Enable secrets engine. `opts`:
...
- `c.token:create(opts?)` -> `{client_token, ...}` -- Create new token. `opts`:
  `{policies, ttl, ...}`
...
- `c.transit:encrypt(key_name, plaintext)` -> ciphertext|nil -- Encrypt with transit engine (auto
  base64 encodes)
- `c.transit:decrypt(key_name, ciphertext)` -> plaintext|nil -- Decrypt with transit engine (auto
  base64 decodes)
...
- `c.pki:issue(mount, role_name, opts?)` -> cert|nil -- Issue certificate. `opts`:
  `{common_name, ttl, ...}`
...
- `M.wait(url, opts?)` -> true -- Wait for Vault to become healthy. `opts`:
