...
- `c.queries:instant(promql)` → number|[{metric, value}] — Instant PromQL query. Single result
  returns number, multiple returns array.
- `c.queries:range(promql, start_time, end_time, step)` → [result] — Range PromQL query over time
  window
- `c.alerts:list()` → [alert] — List active alerts
- `c.targets:list()` → `{activeTargets, droppedTargets}` — List scrape targets with health status
- `c.targets:metadata(opts?)` → [metadata] — Get targets metadata. `opts`:
  `{match_target, metric, limit}`
- `c.rules:list(opts?)` → [group] — List alerting/recording rules. `opts.type` filters by `"alert"`
  or `"record"`.
- `c.labels:values(label_name)` → [string] — List all values for a label name
- `c.series:list(match_selectors)` → [series] — Query series metadata. `match_selectors` is array of
  selectors.
