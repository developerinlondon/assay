...
- `c.apps:refresh(name, opts?)` → app — Refresh app state. `opts.type`: `"normal"` (default) or
  `"hard"`
...
- `c.apps:wait_healthy(name, timeout_secs)` → true — Wait for app to become healthy, errors on
  timeout
