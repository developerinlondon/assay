...
**Driving design spec:** Lives in the consumer (knowhere) repo at
`knowhere/.claude/plans/11-package-manager-design.md`. That spec covers the full host+container
package manager including knowhere's UI; this plan implements only the assay-side framework. The
knowhere-side implementation is planned separately as `knowhere/.claude/plans/13-*` after this plan
ships.
...
(End-to-end test against a real nspawn machine is part of knowhere plan 13's smoke; here we only
...
Operations that actually invoke `apt-get` are tested manually in knowhere plan 13 against a
throwaway nspawn.
...
- `pkg.query` / `pkg.query_all` — depend on knowing whether the target is host or machine and what
  `--version` invocation a given binary supports. knowhere plan 13 implements via
  ...
  Stubs left in place.
- `pkg.apply` / `pkg.reconcile` — orchestration with logging, audit, locking. Implemented in
  knowhere plan 13's `services/host/packages.lua` because the locking + audit emission is
  knowhere-specific.
- The reconciler **second-run idempotency rule** (apply twice → second run is no-op) is verified
  end-to-end in knowhere plan 13's smoke. The pure-value piece (plan idempotency) is covered by
  Task 13.
...
3. **GitHub release-API rate limit.** Anonymous 60 req/hr. This plan doesn't make any release-API
   calls itself (those happen in knowhere plan 13's reconciler). Flagged for knowhere plan 13 to add
   `GITHUB_TOKEN` env support if observed in practice.
...
5. **`asset_pattern` placeholders.** Resolved: `{tag}` is the literal release tag (e.g. `v0.11.0`),
   ...
   to upstream conventions (`x86_64`/`aarch64` straight through; `amd64`/`arm64` mapping handled in
   knowhere plan 13's reconciler). Validator rejects unknown placeholders (Task 8).

6. **dpkg lock contention.** This plan's `apt.install` returns a result table with `status` and
   `stderr` rather than retrying internally. knowhere plan 13's reconciler can implement bounded
...
- **`pkg.plan` NEVER emits remove ops:** removal is a separate explicit operation in knowhere's
  reconciler.
