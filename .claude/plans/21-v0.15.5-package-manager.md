# Plan 21 · v0.15.5 Package Manager — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development
> (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use
> checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add to assay the five Rust builtin extensions (`apt`, `http.download`, `crypto.hash_file`,
`compress.untar`, `systemd.machine_exec`) and the `assay.pkg` Lua stdlib module that together
provide a host-and-container package management framework. Cuts assay v0.15.5.

**Architecture:** Five small builtin additions to existing `crates/assay/src/lua/builtins/` files
(one new: `apt.rs`), plus one new Lua stdlib module at `crates/assay/stdlib/pkg.lua`. All Rust
additions follow assay's existing `register_<name>` registration pattern. The Lua stdlib follows the
existing flat-file convention used by `tailscale.lua`, `traefik.lua`, etc. — single file unless it
grows past ~1000 lines.

**Tech Stack:** Rust 1.95, mlua 0.11.6, tokio 1.49, reqwest 0.13.2 (already pulled), sha2 0.10
(already pulled), flate2/xz2/zstd (already pulled), `tar = "0.4.45"` (NEW dep), `toml = "0.9.12"`
(already pulled). assay version 0.15.4 → 0.15.5 (patch bump).
...
- **`assert` is registered as a TABLE** (assert.eq, assert.not_nil, etc.), shadowing Lua's built-in
  `assert(cond, msg)` function call. Lua tests under `tests/pkg_lua/*.lua` must use a local helper:
  ```lua
  local function check(cond, msg)
    if not cond then error(msg, 2) end
  end
  ```
- **`os.getenv` is NOT exposed.** assay shadows the `os` table with a custom one providing only
  `hostname`, `arch`, `platform`, `time`, `clock`, `date`. Tests needing tmpdir paths should use
  `/tmp` directly + `os.time()` + `math.random(...)`.
- **`fs.list` returns `{name, type}` objects, not strings.** Iterate with
  `for _, entry in ipairs(fs.list(dir)) do ... entry.name ... end`.
- **`toml` builtin's parser is `toml.parse(str)`, not `toml.decode(str)`.**
- **String pattern gotcha:** Lua's `string.find` interprets `-` as a lazy quantifier. Use
  `:find(needle, 1, true)` for literal-substring matching.
- **Test cwd is the crate root** (`crates/assay/`), not the workspace root. Test paths must be
  relative: `tests/pkg_lua/...`, `tests/fixtures/...`.

**Driving design spec:** Lives in the consumer repo at
`consumer/.claude/plans/11-package-manager-design.md`. That spec covers the full host+container
package manager including the consumer's UI; this plan implements only the assay-side framework. The
consumer-side implementation is planned separately as `consumer/.claude/plans/13-*` after this plan
ships.
...
This file structure decomposes responsibility per builtin: each Rust file gains a focused addition,
each test file exercises one builtin, and the Lua stdlib gets one harness that runs all
framework-level tests. Splitting tests per builtin keeps each task focused on a single concern.

---

## Tasks

> Each task ends with a commit. Use `cargo test -p assay --test <test-name>` for targeted runs and
> `cargo test -p assay` for full suites. The `assay` crate is at `crates/assay/`; tests live under
> `crates/assay/tests/`.
...
Edit `crates/assay/Cargo.toml`. Locate the `[dependencies]` section and add `tar = "0.4.45"` near
the other compression-adjacent deps (alphabetical order is fine):
...
Run: `cargo build -p assay` Expected: compiles cleanly. The `tar` crate has zero transitive deps, so
the build delta is small.

- [ ] **Step 3: Commit**

```bash
git add crates/assay/Cargo.toml crates/assay/Cargo.lock
git commit -m "build(deps): add tar 0.4.45 + bump assay to 0.15.5 for pkg builtins"
```
...
The existing `crypto.hash` operates on Lua strings — wasteful for multi-MB binaries. Add a
file-based variant that streams the file in chunks.
...
Run: `cargo test -p assay --test builtins_crypto_hash_file -- --nocapture` Expected: FAIL —
`crypto.hash_file` is nil; tests error with "attempt to call a nil value".

- [ ] **Step 3: Implement `crypto.hash_file`**

Open `crates/assay/src/lua/builtins/crypto.rs`. Locate the `register_crypto` function and the
existing `hash_fn` registration. **Just after** `crypto_table.set("hash", hash_fn)?;` (around line
324), insert:
...
Run: `cargo test -p assay --test builtins_crypto_hash_file` Expected: 5 PASS.

If `test_hash_file_large_file_chunked` fails with a hash mismatch, run
`dd if=/dev/zero bs=1M count=5 status=none | sha256sum` on the dev box and substitute that exact hex
string into the assertion.
...
Existing `http.get/post/...` are in-memory (return body as string). Add
`http.download(url, dest_path, opts)` that streams the response to disk — required for binary
releases that can be tens of MB.
...
Run: `cargo test -p assay --test builtins_http_download` Expected: FAIL — `http.download` is nil.

- [ ] **Step 3: Implement `http.download`**

Open `crates/assay/src/lua/builtins/http.rs`. Locate `register_http`. **At the end of
`register_http`, just before the final `Ok(())`**, add:
...
The `client` capture refers to the `reqwest::Client` already passed into `register_http`. The
closure clones it once per call, matching the existing `http.get` pattern.

- [ ] **Step 4: Run tests, verify they pass**

Run: `cargo test -p assay --test builtins_http_download` Expected: 4 PASS.
...
Reads a tar archive from disk (auto-detecting `.gz`, `.xz`, `.zst` by filename extension or by
passing an explicit `compression` opt) and writes one named member to a destination path. Designed
for "extract `<binary>` from `<thing>.tar.gz` into `<install_path>`".
...
Run: `cargo test -p assay --test builtins_compress_untar` Expected: FAIL — `compress.untar` is nil.

- [ ] **Step 3: Implement `compress.untar`**

Open `crates/assay/src/lua/builtins/compress.rs`. Replace the entire `register_compress` function
with:
...
Open `crates/assay/Cargo.toml`. The `tar` dep added in Task 1 is already in `[dependencies]` —
accessible from tests too, no extra entry needed. Verify by grepping:

Run: `grep -n '^tar' crates/assay/Cargo.toml` Expected: shows `tar = "0.4.45"` once.

- [ ] **Step 5: Run tests, verify they pass**

Run: `cargo test -p assay --test builtins_compress_untar` Expected: 4 PASS.
...
Wraps `systemd-run --machine=<name> --pty --quiet --wait --collect <cmd>`. Returns
`{status, stdout, stderr}` matching the existing `shell.exec` shape. Linux-only; non-Linux registers
a stub that errors at runtime.
...
(End-to-end test against a real nspawn machine is part of the consumer's plan 13 smoke; here we only
verify registration and graceful failure.)

- [ ] **Step 2: Run test, verify it fails**

Run: `cargo test -p assay --test builtins_systemd_machine_exec` Expected: FAIL — function not
registered yet.

- [ ] **Step 3: Implement `systemd.machine_exec`**

Open `crates/assay/src/lua/builtins/systemd.rs`. Locate the list of stub names in the non-Linux
branch (around line 38, the array containing `"list_machines"`, `"machine_status"`, etc.). Add
`"machine_exec"` to the array:
...
Then locate the Linux `register` function (around line 178,
`pub fn register(lua: &Lua, t: &Table) -> mlua::Result<()>`). After `register_journal(lua, t)?;`,
add a call to a new function:
...
(Replace `--pty` from the design doc with `--pipe`. `--pty` allocates a TTY which conflicts with
`--wait` and complicates output capture; `--pipe` is the right flag for "run a command and collect
its stdout/stderr".)

- [ ] **Step 4: Run tests, verify they pass**

Run: `cargo test -p assay --test builtins_systemd_machine_exec` Expected: 2 PASS. The second test
exits with non-zero status (machine doesn't exist) and we assert that fact.
...
The largest of the new builtins. Implements `apt.query`, `apt.list_installed`,
`apt.list_upgradable`, `apt.add_source`, `apt.update`, `apt.install`, `apt.remove`. All operations
are wrappers over `apt-get`, `dpkg-query`, or filesystem writes — none reach into apt's internals.

The strategy for testing: most apt operations require root and a Debian box, which test envs can't
reliably provide. We test:

- **Pure parsers** (apt-list output, dpkg-query output) with sample inputs as fixtures — full unit
  coverage.
- **Function registration** (every method is callable with a string arg) — smoke level.
- **`add_source` content generation** — write to a tempdir, verify file content + idempotent
  re-write.

Operations that actually invoke `apt-get` are tested manually against a throwaway nspawn.
...
Run: `cargo test -p assay --test builtins_apt` Expected: FAIL — `apt` global is nil.
...
//! throwaway nspawn machine in the consumer's plan 13 smoke.
...
Run: `cargo test -p assay --test builtins_apt` Expected: 4 PASS.
...
These TOML files exercise the catalog/template loaders. Created up front so subsequent tasks can
refer to them.
...
Expected: every file prints `ok`. (Even the "invalid" fixtures are syntactically valid TOML —
they're invalid against our schema, not against TOML.)
...
The first slice of the Lua framework. Catalog loader reads TOML files from a list of paths in
priority order, applies the layering rules, validates each entry, and returns `{entries, errors}`.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_catalog_validation` Expected: FAIL — `assay.pkg`
module doesn't exist; the require errors.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_catalog_validation` Expected: PASS — prints
`catalog_validation.lua OK`.
...
Verifies the **last-wins, full-entry override** layering rule and `_origin` tagging across multiple
layers.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_catalog_layering` Expected: PASS.
...
`pkg.templates.load` follows the same layering as catalog. Cross-validation: every package id
referenced by a template must exist in the loaded catalog; missing references produce errors but the
valid templates still load.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_templates_loader` Expected: FAIL —
`pkg.templates.load` is nil.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_templates_loader` Expected: PASS.
...
Used by the reconciler to decide "is `installed_version` < `available_version`?". Handles common
formats:
...
Out of scope for v1: rc/beta/build-metadata, debian-style epoch:upstream-revision (we're comparing
release tags from upstream APIs, not Debian package versions — that's apt's job).
...
Run: `cargo test -p assay --test stdlib_pkg pkg_version_compare` Expected: FAIL.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_version_compare` Expected: PASS.
...
Targets are thin wrappers that let downstream code (`pkg.query`, `pkg.apply`) treat host operations
and machine operations uniformly. A target exposes `:exec(cmd, opts)` which routes to either
`shell.exec` (host) or `systemd.machine_exec` (nspawn).
...
Run: `cargo test -p assay --test stdlib_pkg pkg_target_host_machine` Expected: FAIL — targets are
empty stub table.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_target_host_machine` Expected: PASS. The
non-existent machine test passes because `systemd.machine_exec` from Task 5 returns a result table
with non-zero status rather than throwing.
...
Pure-value plan builder: given catalog + desired set + actual state, produce an array of
`{op, id, method, ...}` operations. This is the heart of the reconciler's idempotency contract —
re-running plan with the same inputs MUST produce the same output, and re-planning after applying
the previous plan MUST produce an empty plan.
...
Run: `cargo test -p assay --test stdlib_pkg pkg_plan_idempotency` Expected: FAIL — `pkg.plan` raises
"not implemented yet".

- [ ] **Step 3: Implement**

In `crates/assay/stdlib/pkg.lua`, replace
`function M.plan() error("pkg.plan: not implemented yet") end` with:
...
Run: `cargo test -p assay --test stdlib_pkg pkg_plan_idempotency` Expected: PASS.
...
Run: `cargo test -p assay` Expected: all tests pass (the existing ones + the seven new test files).

- [ ] **Step 2: Run clippy**

Run: `cargo clippy -p assay --all-targets -- -D warnings` Expected: no warnings. Fix any that appear
(likely cosmetic — unused imports, redundant clones).

- [ ] **Step 3: Run rustfmt**

Run: `cargo fmt -p assay` Expected: no diff. If diff appears, commit it as a separate
`style: cargo fmt` commit.

- [ ] **Step 4: Verify version bump landed**

Run: `grep -n '^version' crates/assay/Cargo.toml` Expected: `version = "0.15.5"`.
...
- `crypto.hash_file(path, algo)`: file-streamed hashing, avoids loading multi-MB binaries into Lua
  strings.
...
- `systemd.machine_exec(name, cmd, opts)`: runs a command inside an nspawn machine via
  `systemd-run --machine=<name> --pipe --quiet --wait --collect`.
- `assay.pkg` Lua stdlib: catalog/template loaders with three-layer overlay (built-in / plugin /
  operator), version comparator, host/machine target abstractions, deterministic plan generator.
  Foundation for the consumer's package management UI.
```
...
git commit -m "release: prep 0.15.5 changelog for package-manager builtins"
```

- [ ] **Step 7: Verify branch state**

Run: `git log --oneline | head -20` Expected: chronological list of all task commits, with the
...
- [x] Five Rust builtins (`apt`, `http.download`, `crypto.hash_file`, `compress.untar`,
      `systemd.machine_exec`) — Tasks 2–6.
- [x] `assay.pkg` Lua stdlib — Tasks 8–13 (catalog loader, layering, templates, version, targets,
      plan).
- [x] Idempotency contract rule 1 (re-plan converged state → empty plan) — Task 13 explicit
      assertion.
- [x] Idempotency contract rule 3 (re-load with no source changes → identical structure) — Task 9
      explicit assertion.
- [x] "Reconcile never removes" — Task 13 explicit assertion (`plan_no_remove`).
- [x] Three-layer catalog overlay — Task 9 explicit assertion (`_origin` tagging across built-in /
      empty plugin / operator).
- [x] Cross-validation: template references → catalog ids — Task 10.
- [x] Atomic file writes (`add_source`, `http.download`, `compress.untar`) — Tasks 3, 4, 6 use
      temp+rename.

**Open items deferred to consumer's plan 13:**

- `pkg.query` / `pkg.query_all` — depend on knowing whether the target is host or machine and what
  `--version` invocation a given binary supports. The consumer's plan 13 implements via
  `services/host/packages.lua` calling `apt.query` directly for apt method and a binary-method
  version probe (e.g. `<install_path> --version` parsed for the first version-shaped substring).
  Stubs left in place.
- `pkg.apply` / `pkg.reconcile` — orchestration with logging, audit, locking. Implemented in the
  consumer's `services/host/packages.lua` because the locking + audit emission is consumer-specific.
- The reconciler **second-run idempotency rule** (apply twice → second run is no-op) is verified
  end-to-end in the consumer's plan 13 smoke. The pure-value piece (plan idempotency) is covered by
  Task 13.

**Placeholder scan** — no "TBD", "implement later", or `// FIXME:`. Each Lua function stub left at
end of pkg.lua (`pkg.query`, `pkg.apply`, etc.) raises an explicit error so callers know the stub is
unimplemented in this plan.

**Type consistency** — every place a builtin is referenced (e.g. `systemd.machine_exec` in Task 5
and again in Task 12's target wrapper, `crypto.hash_file` in Task 2 and again in the spec's
binary-install flow) uses identical names and arg shapes.

**Scope check** — single plan, ~14 tasks, ~5 days of careful work. Within budget.

**Ambiguity check** — every step has either an exact code block or an exact command + expected
output. The one judgment call I flagged inline (the 5MB-zeros sha in Task 2 step 1 may need
recomputation) is documented at the verify step.
...
1. **Stdlib loader and submodule paths.** Confirmed via `crates/assay/src/lua/mod.rs:13`
   (`STDLIB_DIR: Dir = include_dir!("$CARGO_MANIFEST_DIR/stdlib")`). The loader supports both flat
   and dotted paths. v1 keeps everything in a single `pkg.lua` file (well under 1000 lines); split
   deferred.

2. **`tar` crate.** `tar = "0.4.45"` API matches needs: `tar::Archive::new(reader).entries()` yields
   entries with `.path()` and `Read` access — exactly the streaming-extract pattern Task 4 uses.
   Zero transitive deps. Locked in Task 1.

3. **GitHub release-API rate limit.** Anonymous 60 req/hr. This plan doesn't make any release-API
   calls itself (those happen in the consumer's plan 13 reconciler). Flagged for the consumer's plan
   13 to add `GITHUB_TOKEN` env support if observed in practice.

4. **`systemd.machine_exec` invocation shape.** Resolved to `--pipe --quiet --wait --collect` (NOT
   `--pty`). `--pipe` cleanly captures stdout/stderr; `--pty` allocates a TTY which conflicts with
   `--wait` semantics. Documented in Task 5 step 3.

5. **`asset_pattern` placeholders.** Resolved: `{tag}` is the literal release tag (e.g. `v0.11.0`),
   `{ver}` is `{tag}` with leading `v` stripped (e.g. `0.11.0`), `{arch}` is `os_info.arch()` mapped
   to upstream conventions (`x86_64`/`aarch64` straight through; `amd64`/`arm64` mapping handled by
   the consumer's reconciler). Validator rejects unknown placeholders (Task 8).

6. **dpkg lock contention.** This plan's `apt.install` returns a result table with `status` and
   `stderr` rather than retrying internally. The consumer's reconciler can implement bounded
   retry-with-backoff in Lua if needed. Keeps the builtin simple.

## ⚠️ Behavior contracts established by execution

These are decisions made during T8–T13 that the consumer's reconciler must respect:

- **Strict-override semantics:** when a layered overlay (e.g. operator override) provides an INVALID
  entry for an id that an earlier layer provided VALIDLY, the result is `entries[id] = nil` plus an
  error in the errors array. Operator's intent surfaces as a missing entry, not silent fallback to
  the built-in.
- **`pkg.plan` tolerates `actual = nil`** (treated as empty actual map — every desired id becomes an
  install op).
- **`pkg.plan` op shapes:**
  - `{op="install", id, method, target_version}` (target_version may be nil; reconciler resolves at
    apply time)
  - `{op="upgrade", id, method, from, to}`
  - `{op="skip", id, method=nil, reason}` (catalog has no entry for id)
- **`pkg.plan` is deterministic by id-sort:** desired_set order is irrelevant; output is in
  catalog-id alphabetical order.
- **`pkg.plan` NEVER emits remove ops:** removal is a separate explicit operation in the consumer's
  reconciler.
- **`Target:exec(cmd, opts)` returns the `shell.exec` shape:** `{status, stdout, stderr, timed_out}`
  whether host or machine target. `timed_out=true` is set instead of throwing on timeout.
