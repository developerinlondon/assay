...
with 57 modules for Kubernetes, monitoring, secrets, and AI agents.
...
~15 MB, `assay-engine` ~22 MB (the engine grew with the auth + IdP work in v0.2.0).
...
jobs. The runtime talks to a deployed `assay-engine` over HTTP via the `assay.engine.workflow` and
`assay.engine.auth` stdlib modules — same binary, same builtins.
...
| `assay` runtime               | Python / Node + kubectl   | ~15 MB, 5 ms cold start, 57 stdlib modules             |
...
| **assay-engine** | **~16 MB** | **22 MB** |  **1x**  |   **8 ms** |  **Yes**   |
