...
async fn namespace_delete_and_stats(#[case] backend: Backend) {
    ...
    let list = h.list_namespaces().await.unwrap();
    assert!(
        list.iter().any(|n| n.name == ns),
        "namespace should exist after create"
    );
...
async fn workflow_status_transitions(#[case] backend: Backend) {
    let h = backend.setup().await.expect("setup");
    let id = uid("wf-st");
    h.create_workflow(&make_workflow(&id, "main", "main"))
        .await
        .unwrap();
    ...
    h.update_workflow_status(
        &id,
        assay_domain::types::WorkflowStatus::Running,
        None,
        None,
    )
    .await
    ...
    assert!(
        wf.completed_at.is_none(),
        "completed_at should be None while running"
    );
...
async fn workflow_list_with_filters(#[case] backend: Backend) {
    ...
    let all = h
        .list_workflows(&ns, None, None, None, 100, 0)
        .await
        .unwrap();
    assert_eq!(all.len(), 2);
    ...
    let alpha = h
        .list_workflows(&ns, None, Some("type-alpha"), None, 100, 0)
        .await
        .unwrap();
    ...
    let pending = h
        .list_workflows(
            &ns,
            Some(assay_domain::types::WorkflowStatus::Pending),
            None,
            None,
            100,
            0,
    ...
    let running = h
        .list_workflows(
            &ns,
            Some(assay_domain::types::WorkflowStatus::Running),
            None,
            None,
            100,
            0,
...
async fn workflow_task_dispatch(#[case] backend: Backend) {
    ...
    let none = h
        .claim_workflow_task("dispatch-queue", "worker-1")
        .await
        .unwrap();
    ...
    let none2 = h
        .claim_workflow_task("dispatch-queue", "worker-2")
        .await
        .unwrap();
    assert!(
        none2.is_none(),
        "second worker should not claim already-claimed task"
    );
    ...
    assert!(
        claimed2.is_some(),
        "should be claimable after release+re-mark"
    );
...
async fn events_append_and_list(#[case] backend: Backend) {
    let h = backend.setup().await.expect("setup");
    let wf_id = uid("wf-ev");
    h.create_workflow(&make_workflow(&wf_id, "main", "main"))
        .await
        .unwrap();
...
async fn activity_create_and_claim(#[case] backend: Backend) {
    ...
    h.create_workflow(&make_workflow(&wf_id, "main", "act-queue"))
        .await
        .unwrap();
    ...
    let got2 = h
        .get_activity_by_workflow_seq(&wf_id, 1)
        .await
        .unwrap()
        .expect("should exist by seq");
...
async fn activity_retry_on_failure(#[case] backend: Backend) {
    ...
    let claimed = h
        .claim_activity("retry-q", "worker-x")
        .await
        .unwrap()
        .expect("should claim");
    assert_eq!(claimed.id, Some(id));
    h.complete_activity(id, None, Some("transient error"), true)
        .await
        .unwrap();
    ...
    assert!(
        requeued.error.is_none(),
        "error should be cleared on requeue"
    );
    assert!(
        requeued.claimed_by.is_none(),
        "claimed_by should be cleared on requeue"
    );
}
...
async fn activity_heartbeat_timeout(#[case] backend: Backend) {
    ...
    h.create_workflow(&make_workflow(&wf_id, "main", "hb-q"))
        .await
        .unwrap();
    ...
    h.claim_activity("hb-q", "worker-hb")
        .await
        .unwrap()
        .expect("should claim");
...
async fn activity_cancel_pending(#[case] backend: Backend) {
    ...
    h.create_workflow(&make_workflow(&wf_id, "main", "cancel-q"))
        .await
        .unwrap();
    ...
    let id1 = h
        .create_activity(&make_activity(&wf_id, 1, "cancel-q"))
        .await
        .unwrap();
    let id2 = h
        .create_activity(&make_activity(&wf_id, 2, "cancel-q"))
        .await
        .unwrap();
    let id3 = h
        .create_activity(&make_activity(&wf_id, 3, "cancel-claim-q"))
        .await
        .unwrap();

    // Claim activity 3 (on its own queue) → it becomes RUNNING.
    let claimed = h
        .claim_activity("cancel-claim-q", "worker-z")
        .await
        .unwrap();
    ...
    assert_eq!(
        cancelled, 2,
        "should cancel exactly the 2 PENDING activities"
    );
    ...
    assert_eq!(
        a3.status, "RUNNING",
        "RUNNING activity must not be cancelled"
    );
}
...
async fn timer_create_and_fire(#[case] backend: Backend) {
    ...
    let got = h
        .get_timer_by_workflow_seq(&wf_id, 1)
        .await
        .unwrap()
        .expect("should exist");
...
async fn timer_idempotent_on_seq(#[case] backend: Backend) {
    ...
    assert_eq!(
        id1, id2,
        "creating the same (workflow_id, seq) timer twice should return same id"
    );
...
async fn signal_send_and_consume(#[case] backend: Backend) {
    ...
    assert!(
        consumed.iter().all(|s| s.consumed),
        "all returned signals must be marked consumed"
    );
...
async fn schedule_crud(#[case] backend: Backend) {
    ...
    assert!(
        list.iter().any(|s| s.name == sched_name),
    ...
    let updated = h
        .update_schedule("main", &sched_name, &patch)
        .await
        .unwrap();
    assert!(
        updated.is_some(),
        "update_schedule should return updated record"
    );
    assert_eq!(updated.unwrap().cron_expr, "0 0 * * *");

    // set_paused = true
    let paused = h
        .set_schedule_paused("main", &sched_name, true)
        .await
        .unwrap();
    assert!(paused.is_some());
    assert!(
        paused.unwrap().paused,
        "should be paused after set_schedule_paused(true)"
    );
    ...
    // set_paused = false
    let resumed = h
        .set_schedule_paused("main", &sched_name, false)
        .await
        .unwrap();
...
async fn schedule_last_run_update(#[case] backend: Backend) {
    ...
    h.create_schedule(&make_schedule("main", &sched_name))
        .await
        .unwrap();
...
async fn snapshot_create_and_get_latest(#[case] backend: Backend) {
    ...
    let latest = h
        .get_latest_snapshot(&wf_id)
        .await
        .unwrap()
        .expect("should have latest");
    assert_eq!(
        latest.event_seq, 5,
        "get_latest should return highest event_seq"
    );
    ...
    h.create_snapshot(&wf_id, 5, r#"{"step":5,"updated":true}"#)
        .await
        .unwrap();
...
async fn archival_end_to_end(#[case] backend: Backend) {
    ...
    h.create_activity(&make_activity(&wf_id, 1, "main"))
        .await
        .unwrap();
    h.create_timer(&make_timer(&wf_id, 1, 9_999_999_999.0))
        .await
        .unwrap();
    ...
    let wf = h
        ...
        .await
        .unwrap()
        .expect("workflow record must remain");
    assert!(wf.archived_at.is_some(), "archived_at should be set");
    ...
    assert!(
        !archivable2.iter().any(|w| w.id == wf_id),
...
async fn worker_remove_dead(#[case] backend: Backend) {
    ...
    h.heartbeat_worker(&id_live, base_time + 200.0)
        .await
        .unwrap();
    ...
    assert_eq!(
        removed.len(),
        1,
        "exactly one dead worker should be removed"
    );
    assert_eq!(
        removed[0], id_dead,
        "the removed id should be the dead worker"
    );
...
async fn queue_stats(#[case] backend: Backend) {
    ...
    assert_eq!(
        alpha.pending_activities, 2,
        "queue-alpha should have 2 pending activities"
    );
    assert_eq!(
        alpha.running_activities, 0,
        "queue-alpha should have 0 running activities"
    );
    assert_eq!(alpha.workers, 2, "queue-alpha should have 2 workers");

    assert_eq!(
        beta.pending_activities, 1,
        "queue-beta should have 1 pending activity"
    );
    assert_eq!(
        beta.running_activities, 0,
        "queue-beta should have 0 running activities"
    );
...
async fn child_workflows_listing(#[case] backend: Backend) {
    ...
    assert!(
        none.is_empty(),
        "list_child_workflows with unknown parent should return empty"
    );
}
...
async fn leader_election(#[case] backend: Backend) {
    ...
    if matches!(h, common::harness::Harness::Sqlite { .. }) {
        assert!(
            second,
            "SQLite: second acquire should be true (always leader)"
        );
    }
