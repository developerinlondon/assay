...
/// V7: adds `auth.passkeys.passkey_json TEXT` — the full serialised
...
pub const MIGRATION_VERSION: i32 = 7;
...
/// Postgres DDL for the auth schema, version 7 — passkey blob column.
///
...
/// SQLite DDL for the auth schema, version 7 — passkey blob column.
///
/// Mirrors [`PG_DDL_V7`]: adds `passkey_json TEXT` to `auth.passkeys`
...
/// fresh deployments.
pub const SQLITE_DDL_V7: &[(&str, &str)] = &[(
...
///
/// Applies every DDL pack V1–V7 in order. Splits each pack into
...
async fn migrate_postgres_once(pool: &sqlx::PgPool) -> anyhow::Result<()> {
    ...
    for ddl in [
        PG_DDL_V1, PG_DDL_V2, PG_DDL_V3, PG_DDL_V4, PG_DDL_V5, PG_DDL_V6, PG_DDL_V7,
    ] {
...
/// calling. Applies every DDL pack V1–V7 in order. Each DDL chunk is
...
/// (V5, V7) tolerate the "duplicate column name" error SQLite emits on
...
pub async fn migrate_sqlite(pool: &sqlx::SqlitePool) -> anyhow::Result<()> {
    ...
    // V5 and V7 use ALTER TABLE ADD COLUMN which SQLite does not support
    // with IF NOT EXISTS — tolerate duplicate-column errors so re-running
    // on an already-migrated DB is a no-op.
    for (label, stmt) in SQLITE_DDL_V5.iter().chain(SQLITE_DDL_V7) {
...
mod tests {
    ...
    /// V7 upgrade test: simulate a pre-V7 deployment (auth.passkeys table
    ...
    /// scenario real existing deployments face on the first boot after V7.
    #[cfg(feature = "backend-sqlite")]
    #[tokio::test]
    async fn sqlite_v7_upgrade_adds_passkey_json_to_existing_table() {
        use sqlx::Executor;
        ...
        sqlx::query(
            "CREATE TABLE IF NOT EXISTS auth.passkeys (
                ...
                transports TEXT NOT NULL,
                created_at REAL NOT NULL
                -- passkey_json column intentionally absent: pre-V7 state
            )",
        )
        .execute(&pool)
        .await
        .expect("create pre-V7 auth.passkeys");

        // Insert a user row so FK constraints don't block add_passkey.
        sqlx::query("INSERT INTO auth.users (id, created_at) VALUES ('user_upgrade', 0.0)")
            .execute(&pool)
            .await
            .expect("insert test user");

        // Run the migration — V7 ALTER TABLE ADD COLUMN must succeed.
        crate::schema::migrate_sqlite(&pool)
            .await
            .expect("migrate_sqlite V7");

        // Verify the column is present and usable via the store layer.
        use crate::store::UserStore as _;
        let store = crate::store::sqlite::SqliteUserStore::new(pool.clone());
        ...
        store
            ...
            .await
            .expect("add_passkey after V7 upgrade");

        let (owner, fetched) = store
            .get_passkey(&[0xAB, 0xCD])
            .await
            .expect("get_passkey after V7 upgrade")
    ...
    /// Running migrate_sqlite a second time on an already-V7 DB must be
    /// a no-op (duplicate column name is tolerated, not an error).
    #[cfg(feature = "backend-sqlite")]
    #[tokio::test]
    async fn sqlite_v7_migration_is_idempotent() {
        use sqlx::Executor;
        use sqlx::sqlite::{SqliteConnectOptions, SqlitePool, SqlitePoolOptions};
        use std::str::FromStr;
