...
if ! curl -fs "$BASE/api/v1/health" >/dev/null; then
  echo "error: $BASE/api/v1/health unreachable — is the engine running?" >&2
...
for ns in demo prod; do
  curl -fs -X POST "$BASE/api/v1/namespaces" \
...
for i in 1 2 3 4 5; do
  curl -fs -X POST "$BASE/api/v1/workflows" \
    -H 'content-type: application/json' \
    -d "{
...
curl -fs -X POST "$BASE/api/v1/schedules" \
  -H 'content-type: application/json' \
  -d '{
    "namespace":"demo",
    "name":"heartbeat",
...
echo
say "list via API"
echo "  curl $BASE/api/v1/namespaces"
echo "  curl $BASE/api/v1/workflows"
