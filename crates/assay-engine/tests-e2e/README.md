...
`/api/v1/engine/core/info` to answer, runs `assay-engine seed-sample` to populate fixtures, then
drives Playwright. Engine log lands at `/tmp/assay-engine-e2e.log` for post-mortem.
...
- `engine-console.spec.ts`
  ...
  - Modules table loads from `/api/v1/engine/core/modules` (admin token persisted via localStorage
    in beforeEach).
