...
async fn build_auth_ctx_pg(
    ...
    // Zanzibar seed — declarative namespace upsert from a config dir
    // plus an optional bootstrap admin tuple. Idempotent on every
    // boot; see [`AuthZanzibarConfig`] for the knobs.
    #[cfg(feature = "auth-zanzibar")]
    if cfg.auth.zanzibar.namespace_seed_dir.is_some()
        || cfg.auth.zanzibar.bootstrap_admin_email.is_some()
    {
        if let Err(e) = seed_zanzibar(cfg, &ctx).await {
            tracing::warn!("zanzibar seed: {e}");
        }
    }
...
/// Boot-time Zanzibar seeder. Reads `auth.zanzibar.namespace_seed_dir`
/// for `*.json` namespace schemas (one per file) and upserts each via
/// `ZanzibarStore::define_namespace`. Then, if
/// `auth.zanzibar.bootstrap_admin_email` is set, looks up the user by
/// email and writes a `user:<id> → admin → system:<system_object_id>`
/// tuple. Both phases are idempotent (define_namespace replaces;
/// write_tuple is `ON CONFLICT DO NOTHING`).
#[cfg(feature = "auth-zanzibar")]
async fn seed_zanzibar(cfg: &EngineConfig, ctx: &assay_auth::ctx::AuthCtx) -> anyhow::Result<()> {
    let Some(store) = ctx.zanzibar.as_ref() else {
        return Ok(());
    };

    if let Some(dir) = cfg.auth.zanzibar.namespace_seed_dir.as_deref() {
        let rd = match std::fs::read_dir(dir) {
            Ok(rd) => rd,
            Err(e) => {
                tracing::warn!("zanzibar namespace seed dir {dir:?} unreadable: {e}");
                return Ok(());
            }
        };
        ...
        for path in paths {
            let text = match std::fs::read_to_string(&path) {
                Ok(t) => t,
                Err(e) => {
                    tracing::warn!("zanzibar read {path:?}: {e}");
                    continue;
                }
            };
            let schema: assay_auth::zanzibar::NamespaceSchema = match serde_json::from_str(&text) {
                Ok(s) => s,
                Err(e) => {
                    tracing::warn!("zanzibar parse {path:?}: {e}");
                    continue;
                }
            };
            match store.define_namespace(&schema).await {
                Ok(()) => {
                    tracing::info!(
                        "zanzibar namespace seeded: {} (from {})",
                        schema.name,
                        path.display()
                    );
                    count += 1;
                }
                Err(e) => tracing::warn!("zanzibar define {path:?}: {e}"),
            }
        }
        if count > 0 {
            tracing::info!("zanzibar: {count} namespace(s) seeded from {dir}");
        }
    }

    if let Some(email) = cfg.auth.zanzibar.bootstrap_admin_email.as_deref() {
        match ctx.users.get_user_by_email(email).await {
            Ok(Some(user)) => {
                // Bootstrap the user as a full operator across every
                // module the engine fronts. Each tuple corresponds to
                // an existing `require_role_for(ns, obj, rel)` callsite:
                //   auth:<sys>#admin     ← every /admin/* admin route
                //   workflow:<sys>#access ← workflow_gate_middleware
                // Other modules (vault, future namespaces) can be added
                // here as they migrate from admin-key or biscuit auth
                // to session+zanzibar.
                // Each module names its singleton root differently:
                //   auth:<sys> — sys configured via system_object_id
                //                (defaults to "system" — matches what
                ...
                for (object_type, object_id, relation) in tuples {
                    ...
                    match store.write_tuple(&tuple).await {
                        Ok(()) => tracing::info!(
                            "zanzibar bootstrap: user:{} → {} → {}:{}",
                            user.id,
                            tuple.relation,
                            tuple.object_type,
                            tuple.object_id
            ...
            Ok(None) => tracing::warn!(
                "zanzibar bootstrap_admin_email={email:?} not in auth.users; invite the user first, then restart"
            ),
            Err(e) => tracing::warn!("zanzibar bootstrap lookup: {e}"),
        }
    }
