...
async fn build_pg(cfg: EngineConfig, b: crate::init::PgBoot) -> anyhow::Result<EmbeddedEngine> {
    ...
    #[cfg(feature = "vault")]
    let vault_ctx = crate::build_vault_ctx_pg(&cfg.vault.sealing, &b.modules, &b.pool).await?;
...
async fn build_sqlite(
    ...
    #[cfg(feature = "vault")]
    let vault_ctx = crate::build_vault_ctx_sqlite(&cfg.vault.sealing, &b.modules, &b.pool).await?;
    #[cfg(not(feature = "vault"))]
    let vault_ctx: Option<()> = None;
