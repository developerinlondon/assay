...
var AssaySettings = (function () {
  ...
  async function loadNamespaces() {
    ...
    try {
      var namespaces = await fetch('/api/v1/engine/workflow/namespaces').then(function (r) { return r.json(); });
      var statsPromises = namespaces.map(async function (ns) {
        var name = ns.name || ns;
        try {
          var r = await fetch('/api/v1/engine/workflow/namespaces/' + encodeURIComponent(name));
          return await r.json();
        } catch (_) {
  ...
  async function handleCreateNs() {
    ...
    try {
      var res = await fetch('/api/v1/engine/workflow/namespaces', {
        method: 'POST',
  ...
  async function handleDeleteNs(name) {
    ...
    try {
      var res = await fetch('/api/v1/engine/workflow/namespaces/' + encodeURIComponent(name), { method: 'DELETE' });
      if (!res.ok) {
        var body = await res.text();
        throw new Error(body || res.statusText);
      }
