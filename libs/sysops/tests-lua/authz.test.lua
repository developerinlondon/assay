...
-- ---------------------------------------------------------------------
-- Path-boundary semantics: exact public auth routes do NOT apply to
-- subpaths like /auth/login/x. Those subpaths fall through to the
-- broader /auth/ admin rule.
-- ---------------------------------------------------------------------

do
  reset()
  ctx.engine = stub_engine({})
  -- /auth/login is bypass. Path-extending attacks must NOT inherit bypass.
  local r1 = authz.rule_for_path("/auth/login")
  assert.eq(r1 and r1.bypass, true, "/auth/login matches bypass rule")
  local r2 = authz.rule_for_path("/auth/login-attempt")
  assert.eq(r2 and r2.bypass, nil,
            "/auth/login-attempt does NOT inherit /auth/login bypass (segment boundary)")
  local r2b = authz.rule_for_path("/auth/login/x")
  assert.eq(r2b and r2b.bypass, nil,
            "/auth/login/x does NOT inherit /auth/login bypass")
  assert.eq(r2b and r2b.object_type, "auth",
            "/auth/login/x falls through to auth admin rule")
  local r2c = authz.rule_for_path("/auth/callback/x")
  assert.eq(r2c and r2c.bypass, nil,
            "/auth/callback/x does NOT inherit /auth/callback bypass")
  local r2d = authz.rule_for_path("/auth/bootstrap/x")
  assert.eq(r2d and r2d.bypass, nil,
            "/auth/bootstrap/x does NOT inherit /auth/bootstrap bypass")
  local r2e = authz.rule_for_path("/auth/bootstrap")
  assert.eq(r2e and r2e.bypass, nil,
            "/auth/bootstrap is not a public proxy bypass route")
  -- Tree match still works: /auth/users/123/edit matches /auth/users.
  local r3 = authz.rule_for_path("/auth/users/123/edit")
  assert.eq(r3.object_type, "auth", "/auth/users/* matches /auth/users rule")
  reset()
  print("  ok path-boundary semantics: prefix matches require segment break")
end

-- ---------------------------------------------------------------------
-- Trailing-slash prefix still resolves even though mount.lua normalizes —
-- protects against a future caller injecting a raw "/host/" into ctx.
-- ---------------------------------------------------------------------

do
  reset()
  ctx.engine = stub_engine({})
  ctx.prefix = "/host/"  -- deliberately non-canonical
  -- Strict-prefix path under the mount: must be stripped before rule
  -- lookup AND before in_strict_prefix, so we land on no-rule-strict
  -- (fail closed), NOT no-rule (allow).
  local ok, reason = authz.is_allowed("alice@example", "/host/api/v1/engine/nothing/x")
  assert.eq(ok, false,
            "trailing-slash prefix doesn't let unmapped /api/v1/* leak through")
  ctx.prefix = nil
  reset()
  print("  ok trailing-slash prefix is normalized at match time (defense in depth)")
end
