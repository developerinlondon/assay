...
do
  ...
  assert.eq(rows[1].tasks, nil, "service task count is not exposed")
  assert.eq(rows[1].tasks_label, nil, "service task label is not exposed")
  ...
  assert.eq(rows[1].restarts, nil, "service restart count is not exposed")
  ...
  assert.eq(detail_value(rows[1], "Active state"), nil, "service detail omits active state")
  assert.eq(detail_value(rows[1], "Sub state"), nil, "service detail omits sub state")
  ...
  assert.eq(detail_value(rows[1], "Restart policy"), nil, "service detail omits restart policy")
  assert.eq(detail_value(rows[1], "Tasks accounting"), nil, "service detail omits tasks accounting")
