...
do
  ...
  assert.eq(r.body:find("Sub state", 1, true), nil, "services omits sub detail")
  assert.eq(r.body:find("<th>Restarts</th>", 1, true), nil, "services omits restarts column")
  assert.eq(r.body:find("Restart policy", 1, true), nil, "services omits restart policy detail")
  assert.eq(r.body:find("Restart delay", 1, true), nil, "services omits restart delay detail")
  assert.eq(r.body:find(">Description</th>", 1, true), nil, "services omits description column")
  assert.eq(r.body:find("<th>Tasks</th>", 1, true), nil, "services omits tasks column")
  assert.eq(r.body:find("Tasks accounting", 1, true), nil, "services omits tasks accounting detail")
