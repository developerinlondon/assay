...
--!     lib_root = "/opt/assay/libs/sysops",   -- optional, default "."
...
--! through `sysops.ctx` rather than top-level requires.
--!
...
local pages = require("sysops.pages")
...
local function require_table(opts, key)
  ...
  if type(v) ~= "table" then
    error(("sysops.mount: opts.%s must be a table (got %s)"):format(key, type(v)), 3)
  end
  return v
end
...
function M.mount(routes, opts)
  if type(routes) ~= "table" then
    error("sysops.mount(routes, opts): routes must be a table", 2)
  end
