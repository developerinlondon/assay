--! sysops.auth.sessions - session list/revoke SDK for assay-engine auth admin.
--!
--! Wraps: GET    /api/v1/engine/auth/admin/sessions
--!        DELETE /api/v1/engine/auth/admin/sessions/{id}
...
local BASE = "/api/v1/engine/auth/admin/sessions"

local function build_query(opts)
  if not opts or next(opts) == nil then return "" end
  local parts = {}
  for _, k in ipairs({ "user_id", "active_only", "limit" }) do
    if opts[k] ~= nil then
