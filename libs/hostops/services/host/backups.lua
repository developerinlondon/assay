...
-- Talks to rustic_core via the knowhere.rustic.* Lua bindings (Rust-backed).
--
-- The page handlers (pages/backups*.lua) call into this module; this
-- module never directly handles HTTP requests.

local secret_store = require("services.secret_store")
...
function M.test_connection(args)
  ...
  local ok, ret = pcall(knowhere.rustic.test_connection, conn)
...
function M.list_snapshots()
  ...
  local ok, ret = pcall(knowhere.rustic.snapshots, conn)
...
function M.snapshot_detail(snap_id)
  ...
  local ok, ret = pcall(knowhere.rustic.snapshot_detail, conn)
  if not ok then return nil, tostring(ret) end
  return ret
end
...
function M.run_now(args)
  ...
  local function worker()
      ...
      if ok_s and h then
        ...
        for _, src in ipairs(sources) do
          if src:sub(1, #snap_root) == snap_root then
            ...
            table.insert(sources_resolved, h.mount_path .. rel)
          else
    ...
    conn.tags = (function()
      local out = {}
      for _, t in ipairs(tags) do table.insert(out, t) end
      table.insert(out, "manual")
      return out
    end)()
    local ok, ret = pcall(knowhere.rustic.backup, conn)
